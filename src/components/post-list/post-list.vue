<template>
  <view>
      ...
      <view @click="jump(item.id)">
        <view class="post-item">
          <view class="post-item-top-user">
            <view @tap.stop="toUcenter(item.id)">
              <image :src="item.user.avatarUrl"></image>
            </view>
            <view class="user-active">
              <text style="color:#333">{{item.user.nickName}}</text>
              <view v-if="handle && sessionUid == topicUid"
                    @tap.stop="onActive(item,index)"
                    class="arrow-down">
                <u-icon size="40"
                        name="arrow-down"></u-icon>
              </view>
            </view>
            <text style="font-size:10px">{{fomatTime(item.createTime)}}</text>
          </view>
          <text class="discuss-title"
                :data-id="item.discuss_id"
                v-if="item.discuss_id > 0"
          ...
          <view class="p-footer">
            <view v-if="showTopic"
                  @click.stop="jumpTopic(item.circle.id)"
                  class="topic-name">{{item.circle.circleName}}</view>
            <view class="p-right">
              <view class="p-item">
                <text class="iconfont icon-yuedu"></text>
                <text class="count">{{item.glance||0}}</text>
              </view>
    ...
    <u-action-sheet :list="actionList"
                    v-model="showAction"
                    @click="actionCon"></u-action-sheet>
    <!-- 加载状态 -->
