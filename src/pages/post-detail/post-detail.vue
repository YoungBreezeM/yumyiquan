  ...
  <view>
    <view class="info-box">
      <view class="user-item">
        ...
        <block v-if="postDetail.user.id!=user.id">
          <block v-if="postDetail.isAttention">
            <u-button size="mini"
      ...
      <view class="icon">
        <text>{{$f.timeago(postDetail.createTime) }}</text>
      </view>
      <view class="hr"></view>
      <rich-text class="post-text"
                 :nodes="postDetail.content"></rich-text>
      <!-- 图片 -->
      <block v-if="postDetail.type == 1">
        <!--一张图片-->
        <block v-if="postDetail.media.length == 1">
          <image class="img-style-1"
                 @tap.stop="previewImage"
                 mode="aspectFill"
                 :data-current="postDetail.media[0]"
                 :data-image="postDetail.media"
                 :src="postDetail.media[0]"></image>
        </block>
        <!--二张图片-->
        <block v-if="postDetail.media.length == 2">
          <view class="img-style-2">
            <image v-for="(mediaItem, index2) in postDetail.media"
                   :key="index2"
                   @tap.stop="previewImage"
                   mode="aspectFill"
      ...
      <!--按钮-->
      <view class="p-footer">
        <view @click.stop="jumpTopic(postDetail.circleId)"
              class="topic-name">{{postDetail.circle.circleName}}</view>
        <block v-if="postDetail.isCollection">
          <view class="btn m-left-auto m-right-20"
                @click="cancelCollection">
            <text class="iconfont icon-lujing"
                  style="color: #d81e06;"></text>
            <text>喜欢</text>
          </view>
        </block>
        <block v-else>
    ...
    <view class="comment-box">
      <view class="title">评论（{{postDetail.comments.length}}）</view>
      <block v-if="postDetail.comments&&postDetail.comments.length> 0">
...
export default {
  ...
  methods: {
    ...
    cancelCollection () {
      this.$H.post('/yj-grouping/favorites', {
        articleId: this.postId,
        userId: this.user.id
      }).then(res => {
        if (res.code === "00000") {
    ...
    follow () {
      this.$H.post('/yj-grouping/attention', {
        fromId: this.user.id,
        toId: this.postDetail.user.id
      }).then(res => {
        if (res.code === "00000") {
          this.postDetail.isAttention = true;
        } else {
          this.$refs.uToast.show({
            title: res.msg,
            type: 'error'
    ...
    cancelFollow () {
      this.$H.post('/yj-grouping/attention', {
        ...
        if (res.code === "00000") {
          this.postDetail.isAttention = false;
        } else {
          this.$refs.uToast.show({
            title: res.msg,
            type: 'error'
